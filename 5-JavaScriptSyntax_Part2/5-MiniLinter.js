...
storyWords.map(w => {
  let overusedIndex = overusedWords.indexOf(w);
  if(overusedIndex >= 0) {
    overusedCounts[overusedIndex]++;
  }
});

let sentenceCount = 0;
story.split('').forEach(c => {
  if (c === "." || c === "!") {
    sentenceCount++
  }
});

const output = (words, sentences, overused) => {
  ...
  console.log('  Really: ' + overused[0]);
  console.log('  Very: ' + overused[1]);
  console.log('  Basically: ' + overused[2]);
};
output(storyWords.length, sentenceCount, overusedCounts);
